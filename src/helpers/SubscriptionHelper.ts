...
import {
  ...
  unsubscribeFromPush, executeAndTimeoutPromiseAfter, urlBase64ToUint8Array
} from '../utils';
...
export default class SubscriptionHelper {
  ...
  static enableNotifications(existingServiceWorkerRegistration) { // is ServiceWorkerRegistration type
    ...
    return navigator.serviceWorker.ready.then(function (serviceWorkerRegistration) {
      ...
      return SubscriptionHelper.subscribeForPush(serviceWorkerRegistration);
    });
  }
  ...
  static async actuallySubscribeForPush(serviceWorkerRegistration): Subscription {
    // Get stored VAPID public key, originally from OneSignal
    const { vapidPublicKey } = await Database.getAppConfig();

    // Set up how we'd like to subscribe
    const subscribeOptions = {
      userVisibleOnly: true,
      applicationServerKey: undefined
    };
    const shouldSubscribeUsingVapid = Environment.supportsVapid() && !!vapidPublicKey;

    // If we're subscribing by VAPID, set the optional applicationServerKey parameter
    if (shouldSubscribeUsingVapid) {
      // The VAPID public key is base64-URL-encoded, so we have to make it standard base64 before decoding it
      // This is as simple as replacing these two characters
      console.log('Subscribing using VAPID key', vapidPublicKey);
      subscribeOptions.applicationServerKey = urlBase64ToUint8Array(vapidPublicKey);
    }

    // Actually subscribe for push
    const browserSubscription = await serviceWorkerRegistration.pushManager.subscribe(subscribeOptions);

    // Read out push subscription properties
    // Get only the push token and not the beginning of the endpoint; this is for legacy purposes
    const pushToken = browserSubscription.endpoint.substring(browserSubscription.endpoint.lastIndexOf('/') + 1);
    const subscription = await Database.getSubscription();
    subscription.pushToken = pushToken;
    subscription.pushEndpoint = browserSubscription.endpoint;

    if (browserSubscription.getKey) {
      // p256dh and auth are both ArrayBuffer
      let p256dh = null;
      try {
        p256dh = browserSubscription.getKey('p256dh');
      } catch (e) {
        // User is most likely running < Chrome < 50
      }
      let auth = null;
      try {
        auth = browserSubscription.getKey('auth');
      } catch (e) {
      ...
      if (p256dh) {
        ...
        subscription.cryptoP256dh = p256dh_base64encoded;
      }
      if (auth) {
        // Base64 encode the ArrayBuffer (not URL-Safe, using standard Base64)
        let auth_base64encoded = btoa(
          String.fromCharCode.apply(null, new Uint8Array(auth)));
        subscription.cryptoAuth = auth_base64encoded;
      }
    }
  ...
  static async subscribeForPush(serviceWorkerRegistration) {
    log.debug(`Called %c_subscribeForPush()`, getConsoleStyle('code'));

    log.debug(`Calling %cServiceWorkerRegistration.pushManager.subscribe() with 15s timeout`, getConsoleStyle('code'));
    try {
      var subscription: Subscription = await executeAndTimeoutPromiseAfter(
        SubscriptionHelper.actuallySubscribeForPush(serviceWorkerRegistration),
        15000,
      ...
      log.debug(`Finished calling %cServiceWorkerRegistration.pushManager.subscribe()`, getConsoleStyle('code'), subscription);
    } catch (e) {
    ...
    if (OneSignal._thisIsThePopup) {
      ...
      MainHelper.registerWithOneSignal(appId, subscription);
    }
  }
}